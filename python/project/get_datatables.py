def get_datatables_records(request, querySet, columnIndexNameMap, jsonTemplatePath = None,
                  extrafilters={},handle_user=False,updatefilter={},updateitems={}, *args):
    ...
    toutputQ = None
    for col in range(0,cols):
        tSearch = request.GET.get('sSearch_{0}'.format(col), False)
        if tSearch > '':
            # parse the '|' in the search text
            if tSearch.find("|") != -1:
                tContain = tSearch.split('|')
                for sc in tContain:
                    kwargz = {columnIndexNameMap[col]+"__iexact":sc}
                    toutputQ = toutputQ | Q(**kwargz) if toutputQ else Q(**kwargz)
                outputQ = outputQ & toutputQ if outputQ else toutputQ
            else:
                kwargz = {columnIndexNameMap[col]+"__iexact" : request.GET['sSearch_{0}'.format(col)]}
                outputQ = outputQ & Q(**kwargz) if outputQ else Q(**kwargz)
    if outputQ:
        print outputQ
        print "olen=",len(querySet)
        querySet = querySet.filter(outputQ)
        print "plen=",len(querySet)
        ...
        for row in a:
            ...
            for col in range(0,len(colitems)):
                for idx, val in enumerate(rowkeys):
                    if val == colitems[col]:
                        if isinstance(rowvalues[idx],datetime.datetime):
